...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 3
    if num_rolls == 0:
        return abs(opponent_score // 10 - opponent_score % 10) + 2
    else:
...
def make_averaged(fn, num_samples=1000):
    ...
    def average_function(*args):
        result = 0
        number_calls = 0
        while number_calls < num_samples:
            result += fn(*args)
            number_calls +=1
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    n_result = 0
    while n <= 10:
      result = make_averaged(roll_dice) (n,dice)
      if result > compareAvg :
        compareAvg = result
        n_result = n
      n += 1
    return n_result
